...
| **Persist** a fitted model as plain JSON, reload, predict | `save_model` / `load_model` / `predict` | — |
...
X'X matrix stops being an option. The logistic classifier shares that streaming
machinery with `csv_sgd_regression`, differing only in the loss it minimises.
Naive Bayes needs no optimizer at all — its parameters *are* grouped summary
statistics, fitted in one parallel pass with nothing cached. Every classifier
reports accuracy, log-loss, ROC-AUC, and a confusion matrix on the held-out
split, and every fitted parameter is comparable with scikit-learn's on the
same data — the differential tests assert it.
...
The same journey for a binary classifier, across two model families: logistic
regression, where every epoch does real per-row arithmetic, and Gaussian Naive
Bayes, whose parameters *are* grouped summary statistics — one parallel pass,
nothing cached, memory bounded by the feature count alone.

<!-- CLASSIFY:START -->

<!-- Generated by `python -m benches.render --write`. Do not edit by hand. -->

Workload: **CSV on disk → 80/20 split → fitted classifier → held-out metrics**, on 500,000 rows × 20 features (91.6 MB). Labels are *sampled* from `Bernoulli(sigmoid(w·x + b))` rather than thresholded, so the classes overlap — on perfectly separable data the logistic likelihood has no finite maximum and every implementation ‘agrees’ only in diverging. `vs Grizzly` compares within a model family: logistic against logistic, Naive Bayes against Naive Bayes.

| Method | Time | vs Grizzly | Accuracy | ROC-AUC | Peak memory |
|--------|-----:|-----------|---------:|--------:|------------:|
| **Grizzly** logistic SGD (10 epochs) | 245.3 ms | — | 0.8556 | 0.9357 | 200.6 MB |
| pandas → sklearn `LogisticRegression` | 760.0 ms | 3.1x slower | 0.8589 | 0.9380 | 527.8 MB |
| polars → sklearn `LogisticRegression` | 281.3 ms | 1.1x slower | 0.8589 | 0.9380 | 501.5 MB |
| pandas → sklearn `SGDClassifier` (10 epochs) | 1452.0 ms | 5.9x slower | 0.8587 | 0.9373 | 572.1 MB |
| **Grizzly** Gaussian Naive Bayes | 54.8 ms | — | 0.8564 | 0.9364 | 127.0 MB |
| pandas → sklearn `GaussianNB` | 722.9 ms | 13.2x slower | 0.8584 | 0.9377 | 642.8 MB |
| polars → sklearn `GaussianNB` | 259.3 ms | 4.7x slower | 0.8584 | 0.9377 | 602.0 MB |

Held-out metrics agree across every method of each family (logistic: accuracy within 0.0033, ROC-AUC within 0.0023; gnb: accuracy within 0.0020, ROC-AUC within 0.0013), so these are timings of equally good classifiers rather than accidentally different ones.
